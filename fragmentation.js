/**
 * @param ID {String} 要实现动画的效果的元素ID，需带#
 * @param width 容器的宽度，默认值未 100
 * @param height 容器的高度，默认值未 100
 */
function burst(ID, width = 100, height = 100) {
    ...
    var C = 15;
    // 容器宽度
    // var W = width || parseInt(window.getComputedStyle(img)['width']);
    var W = width
    // 容器高度
    // var H = height || parseInt(window.getComputedStyle(img)['height']);
    var H = height
    // 控制碎片的范围
    var N = 2;
    // 碎片分散时，整个活动范围的宽
